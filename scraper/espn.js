...
async function scrapeESPN(ESPNuser, ESPNpass, ESPNleague) {
  ...
  let data = await page.evaluate(() => {
    document.querySelector("a#global-user-trigger").click();
    return {};
  });
  console.log(data);
...
*/
  ...
  await page.goto(
    "https://fantasy.espn.com/basketball/team?leagueId=63980886",
  ...
  await page.evaluate(() => {
    let div = document.querySelector(
      'li[class="standings NavSecondary__Item"]'
    );
  ...
  let counter = await page.evaluate(() => {
    let diver = document.querySelector(
      'div[class="jsx-2972152035 season--stats--table"] >  div > div[class="flex"] > table > tbody'
    );
  ...
  for (var j = 0; j < counter; j += 1) {
    ...
    await page.waitForSelector(
      'table[class="Table Table--align-right Table--fixed Table--fixed-left"] > tbody'
    );
    console.log(j);
    ...
    let oz = await page.evaluate(
      (j, standings, leagueCatAvg) => {
        ...
        let catTable = document.querySelector(
          'div[class="Table__ScrollerWrapper relative overflow-hidden"] > div[class="Table__Scroller"] > table > tbody'
        );
        ...
        for (var y = 0; y < nu; y += 1) {
          if (y == 0) {
            //if its first time around loop, collect cateogry names
            let cat = document.querySelector(
              'div[class="Table__ScrollerWrapper relative overflow-hidden"] > div[class="Table__Scroller"] > table > thead > tr[class="Table__sub-header Table__TR Table__even"]'
            ); //get category row names
          ...
          if (
            playerName.title != "Player" &&
            playerName.hasAttribute("title")
          ) {
    ...
    for (var i = 0; i < cats.length + 2; i++) {
      //loop to format cat averages object
      let cat = cats[i];
      if (oz.catAvg[i] && cat) {
        if (cat === "FTM") {
          let g = oz.catAvg[i];
